  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (player) {
        ...
        if (isRodCast) {
          // ROD CAST: Move hook with both WASD and Arrow keys
          const hookSpeed = 5;
          ...
          if (commands.moveUp || commands.moveDown || commands.moveLeft || commands.moveRight) {
            connection.invoke("MoveHook", player.fishingRod.positionX, player.fishingRod.positionY)
              .catch((err) => console.error(err));
          }
        } else {
          // ROD NOT CAST: Move boat with both A/D and Arrow Left/Right
          ...
          if (commands.moveLeft || commands.moveRight) {
            connection.invoke("MoveBoatTo", player.boat.positionX)
              .catch((err) => console.error(err));
          }
        }
        ...
        if (player.fishingRod.positionX > gameEnvironmentData.width) player.fishingRod.positionX = gameEnvironmentData.width;
        if (player.fishingRod.positionY < 0) player.fishingRod.positionY = 0;
        if (player.fishingRod.positionY > gameEnvironmentData.waterHeight) player.fishingRod.positionY = gameEnvironmentData.waterHeight;

        if (player.boat.positionX < 0) player.boat.positionX = 0;
        if (player.boat.positionX > gameEnvironmentData.width - player.boat.width) player.boat.positionX = gameEnvironmentData.width - player.boat.width;

        // Cast toggle with cooldown
        const now = Date.now();
        if (commands.castTrigger && now - lastCastTimeRef.current > castCooldownMs) {
          ...
          inputAdapter.clearCastTrigger(); // Clear the trigger after use
        }
      }
